...
class CreateText extends CreateElement
{
    ...
    public function createText()
    {
        ...
        if (!empty($args[1]['framePr'])) {
            $this->generateFRAMEPR($args[1]['framePr']);
        }
        if (isset($args[1]['lineSpacing']) ||
            ...
            if (!isset($args[1]['spacingLineRule'])) {
                $args[1]['spacingLineRule'] = 'auto';
            }
            $this->generateSPACING($args[1]['spacingTop'], $args[1]['spacingBottom'], $args[1]['lineSpacing'], $args[1]['spacingLineRule']);
        }
        if (!empty($args[1]['vAlign'])) {
            $this->generateVALIGN($args[1]['vAlign']);
        }
        if (
        ...
        if (!empty($args[1]['numForm'])) {
            $this->generateNUMFORM($args[1]['numForm']);
        }
        if (!empty($args[1]['numSpacing'])) {
            $this->generateNUMSPACING($args[1]['numSpacing']);
        }
        ...
        if (!is_array($args[0])) {
            ...
            foreach ($args[0] as $texts) {
                ...
                if ($texts instanceof WordFragment) {
                    ...
                    if (!empty($texts['numForm'])) {
                        $this->generateNUMFORM($texts['numForm']);
                    }
                    if (!empty($texts['numSpacing'])) {
                        $this->generateNUMSPACING($texts['numSpacing']);
                    }
     ...
     * @param array $framePr
     */
    protected function generateFRAMEPR($framePr = array())
    {
        if (count($framePr) > 0) {
            $xml = '<' . CreateElement::NAMESPACEWORD . ':framePr ';
            foreach ($framePr as $key => $value) {
                $xml .= CreateElement::NAMESPACEWORD . ':' . $key . '="' . $value . '" ';
            }
            $xml .= '></' . CreateElement::NAMESPACEWORD . ':framePr>__PHX=__GENERATEPPR__';
    ...
    /**
     * Generate w14:numForm
     *
     * @access protected
     * @param string $val
     */
    protected function generateNUMFORM($val = 'default')
    {
        if (in_array($val, ['default', 'lining', 'oldStyle'])) {
            $this->_xml = str_replace(
                '__PHX=__GENERATERPR__',
                '<w14:numForm xmlns:w14="http://schemas.microsoft.com/office/word/2010/wordml" w14:val="' . $val . '"/>__PHX=__GENERATERPR__', $this->_xml
            );
        }
    }

    /**
     * Generate w14:numSpacing
     *
     * @access protected
     * @param string $val
     */
    protected function generateNUMSPACING($val = 'default')
    {
        if (in_array($val, ['default', 'proportional', 'tabular'])) {
            $this->_xml = str_replace(
                '__PHX=__GENERATERPR__',
                '<w14:numSpacing xmlns:w14="http://schemas.microsoft.com/office/word/2010/wordml" w14:val="' . $val . '"/>__PHX=__GENERATERPR__', $this->_xml
            );
        }
    }
     ...
     * @access protected
     */
    protected function generateSPACING($spacingTop, $spacingBottom, $line = '240', $lineRule = 'auto')
    {
        ...
        $xml .= CreateElement::NAMESPACEWORD . ':line="' . $line .
                '" ' . CreateElement::NAMESPACEWORD . ':lineRule="' . $lineRule . '"/>__PHX=__GENERATEPPR__';
    ...
    /**
     * Generate w:textAlignment
     *
     * @access protected
     */
    protected function generateVALIGN($val = 'auto')
    {
        $this->_xml = str_replace(
                '__PHX=__GENERATEPPR__', '<' . CreateElement::NAMESPACEWORD .
                ':textAlignment w:val="' . $val . '"></' .
                CreateElement::NAMESPACEWORD . ':textAlignment>__PHX=__GENERATEPPR__', $this->_xml
        );
    }
